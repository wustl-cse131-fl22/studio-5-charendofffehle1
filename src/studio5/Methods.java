...
public class Methods {
	...
	public static double distanceBetween(double x1, double y1, double x2, double y2) {
		double newx = Math.pow((x1-x2),2);
		double newy = Math.pow((y1-y2),2);
	...
	public static void drawBullsEye(double x, double y, double radius) {
		...
		double brad = radius*(3.0/4.0);
		StdDraw.filledCircle(x, y, brad);

		// Red ring with 1.0/2.0 the radius
		// suggested rgb values: 146, 0, 0

		StdDraw.setPenColor(146,0,0);
		double rrad = radius*(1.0/2.0);
		StdDraw.filledCircle(x, y, rrad);

		// Yellow ring with 1.0/4.0 the radius
		// suggested rgb values: 255, 255, 109

		StdDraw.setPenColor(255,255,109);
		double yrad = radius*(1.0/4.0);
		StdDraw.filledCircle(x, y, yrad);
	}
